calc.ipw.trt.eff <- function(data.node, x, ux, prop.sc, propsc.mthd, propsc.form.true){
  ...
  for (i in 1:length(ux)){
    ...
    if (!is.null(prop.sc)) {
      ...
      # If there is only one observation in df.noy, there will be an error if input into gen.fullrank.ipw
      if (nrow(data.sub.node.noy) == 1) {
      ...
      prop.sc.sub <- tmp$prop.sc
      
    }
    
    trt.eff     <- c(trt.eff, mean(data.sub.node$Y * data.sub.node$A / prop.sc.sub) - 
                       mean(data.sub.node$Y * (data.sub.node$A) / (1 - prop.sc.sub)))
...
gen.fullrank.ipw <- function(df.noy, propsc.form.true) {
  ...
  df.noy.fullrank <- df.noy[, c(T, sapply(df.noy[, colnames(df.noy) != "A"], 
  ...
  if (ncol(df.noy.fullrank) < ncol(df.noy) & (!is.null(propsc.form.true))) {
    ...
    propsc.terms <- propsc.terms[propsc.terms.colname %in% names(df.noy.fullrank)]
    propsc.form.true.updated <- paste("A ~ ", paste(propsc.terms, collapse= " + "))
  } else {
