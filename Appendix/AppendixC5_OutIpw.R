...
performance.ipwOut <- 
  foreach(i = start:end,
          .combine  = "rbind") %dopar% {
            ...
            eval.final.estipw.glm.propscout.true.cv1                <- unlist(eval.final.estipw.glm.propscout.true.cv1)
            names(eval.final.estipw.glm.propscout.true.cv1)         <- paste0("hetero.ipw.glm.propscout.true.cv1.", names(eval.final.estipw.glm.propscout.true.cv1))
            print("1.cv1.T")
            ...
            eval.final.estipw.glm.propscout.nois.cv1                <- unlist(eval.final.estipw.glm.propscout.nois.cv1)
            names(eval.final.estipw.glm.propscout.nois.cv1)         <- paste0("hetero.ipw.glm.propscout.nois.cv1.", names(eval.final.estipw.glm.propscout.nois.cv1))
            print("2.cv1.nois")
            
            #####################################################################################################################
            ####################### 3. ipw: GLM Model, outside node, Mis Cov propensity score model, cv1 ########################
            #####################################################################################################################
            ...
            eval.final.estipw.glm.propscout.mis.cv1                <- unlist(eval.final.estipw.glm.propscout.mis.cv1)
            names(eval.final.estipw.glm.propscout.mis.cv1)         <- paste0("hetero.ipw.glm.propscout.mis.cv1.", names(eval.final.estipw.glm.propscout.mis.cv1))
            print("3.cv1.mis")
            ...
            eval.final.estipw.glm.propscout.true.cv2                <- unlist(eval.final.estipw.glm.propscout.true.cv2)
            names(eval.final.estipw.glm.propscout.true.cv2)         <- paste0("hetero.ipw.glm.propscout.true.cv2.", names(eval.final.estipw.glm.propscout.true.cv2))
            print("4.cv2.T")
            ...
            eval.final.estipw.glm.propscout.nois.cv2                <- unlist(eval.final.estipw.glm.propscout.nois.cv2)
            names(eval.final.estipw.glm.propscout.nois.cv2)         <- paste0("hetero.ipw.glm.propscout.nois.cv2.", names(eval.final.estipw.glm.propscout.nois.cv2))
            print("5.cv2.nois")
            
            #####################################################################################################################
            ######################## 6. ipw: GLM Model, outside node, Mis Cov propensity score model, Cv2 #######################
            #####################################################################################################################
            ...
            eval.final.estipw.glm.propscout.mis.cv2                <- unlist(eval.final.estipw.glm.propscout.mis.cv2)
            names(eval.final.estipw.glm.propscout.mis.cv2)         <- paste0("hetero.ipw.glm.propscout.mis.cv2.", names(eval.final.estipw.glm.propscout.mis.cv2))
            print("6.cv2.mis")
            
            performance.hetero.ipw <- c(eval.final.estipw.glm.propscout.true.cv1,
                                        eval.final.estipw.glm.propscout.nois.cv1,
                                        eval.final.estipw.glm.propscout.mis.cv1,
                                        eval.final.estipw.glm.propscout.true.cv2,
                                        eval.final.estipw.glm.propscout.nois.cv2,
                                        eval.final.estipw.glm.propscout.mis.cv2)
            ...
            #####################################################################################################################
            ######################## 7. ipw: GLM Model, outside node, True propensity score model, cv1 ##########################
            #####################################################################################################################
            t0 <- Sys.time()
            seq.created.estipw.glm.propscout.true.cv1 <- create.sequence(data.used         = data.used.cont.cont,
                                                                         est.used          = "IPW",
            ...
            eval.final.estipw.glm.propscout.true.cv1$t <- as.numeric(difftime(t1, t0, units = "secs"))
            eval.final.estipw.glm.propscout.true.cv1                <- unlist(eval.final.estipw.glm.propscout.true.cv1)
            names(eval.final.estipw.glm.propscout.true.cv1)         <- paste0("homo.ipw.glm.propscout.true.cv1.", names(eval.final.estipw.glm.propscout.true.cv1))
            print("7.cv1.T")
            
            #####################################################################################################################
            ###################### 8. ipw: GLM Model, outside node, Mis Func propensity score model, cv1 ########################
            #####################################################################################################################
            t0 <- Sys.time()
            seq.created.estipw.glm.propscout.nois.cv1 <- create.sequence(data.used         = data.used.cont.cont,
                                                                         est.used          = "IPW",
            ...
            eval.final.estipw.glm.propscout.nois.cv1$t <- as.numeric(difftime(t1, t0, units = "secs"))
            eval.final.estipw.glm.propscout.nois.cv1                <- unlist(eval.final.estipw.glm.propscout.nois.cv1)
            names(eval.final.estipw.glm.propscout.nois.cv1)         <- paste0("homo.ipw.glm.propscout.nois.cv1.", names(eval.final.estipw.glm.propscout.nois.cv1))
            print("8.cv1.nois")
            
            #####################################################################################################################
            ####################### 9. ipw: GLM Model, outside node, Mis Cov propensity score model, cv1 ########################
            #####################################################################################################################
            data.used.cont.cont.mis <- data.used.cont.cont %>%
              select(-X2)
            data.validation.cont.cont.mis <- data.validation.cont.cont %>%
            ...
            eval.final.estipw.glm.propscout.mis.cv1$t <- as.numeric(difftime(t1, t0, units = "secs"))
            eval.final.estipw.glm.propscout.mis.cv1                <- unlist(eval.final.estipw.glm.propscout.mis.cv1)
            names(eval.final.estipw.glm.propscout.mis.cv1)         <- paste0("homo.ipw.glm.propscout.mis.cv1.", names(eval.final.estipw.glm.propscout.mis.cv1))
            print("9.cv1.mis")
            
            #####################################################################################################################
            ######################## 10. ipw: GLM Model, outside node, True propensity score model, Cv2 #########################
            #####################################################################################################################
            t0 <- Sys.time()
            seq.created.estipw.glm.propscout.true.cv2 <- create.sequence(data.used         = data.used.full.cont.cont,
                                                                         est.used          = "IPW",
            ...
            eval.final.estipw.glm.propscout.true.cv2$t <- as.numeric(difftime(t1, t0, units = "secs"))
            eval.final.estipw.glm.propscout.true.cv2                <- unlist(eval.final.estipw.glm.propscout.true.cv2)
            names(eval.final.estipw.glm.propscout.true.cv2)         <- paste0("homo.ipw.glm.propscout.true.cv2.", names(eval.final.estipw.glm.propscout.true.cv2))
            print("10.cv2.T")
            
            #####################################################################################################################
            ####################### 11. ipw: GLM Model, outside node, Mis Func propensity score model, Cv2 ######################
            #####################################################################################################################
            t0 <- Sys.time()
            seq.created.estipw.glm.propscout.nois.cv2 <- create.sequence(data.used         = data.used.full.cont.cont,
                                                                         est.used          = "IPW",
            ...
            eval.final.estipw.glm.propscout.nois.cv2$t <- as.numeric(difftime(t1, t0, units = "secs"))
            eval.final.estipw.glm.propscout.nois.cv2                <- unlist(eval.final.estipw.glm.propscout.nois.cv2)
            names(eval.final.estipw.glm.propscout.nois.cv2)         <- paste0("homo.ipw.glm.propscout.nois.cv2.", names(eval.final.estipw.glm.propscout.nois.cv2))
            print("11.cv2.nois")
            
            #####################################################################################################################
            ######################## 12. ipw: GLM Model, outside node, Mis Cov propensity score model, Cv2 ######################
            #####################################################################################################################
            data.used.full.cont.cont.mis <- data.used.full.cont.cont %>%
              select(-X2)
            ...
            eval.final.estipw.glm.propscout.mis.cv2$t <- as.numeric(difftime(t1, t0, units = "secs"))
            eval.final.estipw.glm.propscout.mis.cv2                <- unlist(eval.final.estipw.glm.propscout.mis.cv2)
            names(eval.final.estipw.glm.propscout.mis.cv2)         <- paste0("homo.ipw.glm.propscout.mis.cv2.", names(eval.final.estipw.glm.propscout.mis.cv2))
            print("12.cv2.mis")
            
            performance.homo.ipw <- c(eval.final.estipw.glm.propscout.true.cv1,
                                      eval.final.estipw.glm.propscout.nois.cv1,
                                      eval.final.estipw.glm.propscout.mis.cv1,
                                      eval.final.estipw.glm.propscout.true.cv2,
                                      eval.final.estipw.glm.propscout.nois.cv2,
...
save(performance.ipwOut, file = paste0("../Data/AppendixC5/OutIpw.RData"))
