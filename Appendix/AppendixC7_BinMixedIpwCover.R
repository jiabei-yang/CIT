  ...
  foreach(i = start:end,
          .combine  = "rbind") %dopar% {
            ...
            for (boot.i in 1:boot.R) {
              ...
              ################################################################################
              # True
              hetero.trt.eff[boot.i, 1] <- mean(data.l$Y * data.l$A / propsc.T.l) - 
                mean(data.l$Y * (1 - data.l$A) / (1 - propsc.T.l))
              hetero.trt.eff[boot.i, 2] <- mean(data.r$Y * data.r$A / propsc.T.r) - 
                mean(data.r$Y * (1 - data.r$A) / (1 - propsc.T.r))
              
              # Noisy 
              hetero.trt.eff[boot.i, 3] <- mean(data.l$Y * data.l$A / propsc.Nois.l) - 
                mean(data.l$Y * (1 - data.l$A) / (1 - propsc.Nois.l))
              hetero.trt.eff[boot.i, 4] <- mean(data.r$Y * data.r$A / propsc.Nois.r) - 
                mean(data.r$Y * (1 - data.r$A) / (1 - propsc.Nois.r))
              
              # Mis
              hetero.trt.eff[boot.i, 5] <- mean(data.l$Y * data.l$A / propsc.F.l) - 
                mean(data.l$Y * (1 - data.l$A) / (1 - propsc.F.l))
              hetero.trt.eff[boot.i, 6] <- mean(data.r$Y * data.r$A / propsc.F.r) - 
                mean(data.r$Y * (1 - data.r$A) / (1 - propsc.F.r))
            ...
            for (boot.i in 1:boot.R) {
              ...
              ################################################################################
              # True
              homo.trt.eff[boot.i, 1] <- mean(data.boot$Y * data.boot$A / whl.propsc.T$value$prop.sc) - 
                mean(data.boot$Y * (1 - data.boot$A) / (1 - whl.propsc.T$value$prop.sc))
              
              # Noisy 
              homo.trt.eff[boot.i, 2] <- mean(data.boot$Y * data.boot$A / whl.propsc.Nois$value$prop.sc) - 
                mean(data.boot$Y * (1 - data.boot$A) / (1 - whl.propsc.Nois$value$prop.sc))
              
              # Mis
              homo.trt.eff[boot.i, 3] <- mean(data.boot$Y * data.boot$A / whl.propsc.F$value$prop.sc) - 
                mean(data.boot$Y * (1 - data.boot$A) / (1 - whl.propsc.F$value$prop.sc))
...
save(ci.ipw, file = paste0("../Data/AppendixC7/BinMixedIpwCover.RData"))
