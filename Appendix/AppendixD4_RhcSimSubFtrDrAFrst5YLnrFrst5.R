...
# rhc.used <- rhc.used %>%
#   mutate(sim.A = rbinom(nrow(rhc.used), size = 1, prob = prob.sim.A))
# 
...
#   0.4 * rhc.used$sim.A * (grepl("MOSF", rhc.used$cat1) | (rhc.used$cat1 == "ARF"))
# 
# rhc.used <- rhc.used %>%
...
# 
# rhc.used <- rhc.used %>%
#   mutate(sim.Y.noeff = rbinom(nrow(rhc.used), size = 1, prob = prob.noeff.y),
#          sim.Y.eff   = rbinom(nrow(rhc.used), size = 1, prob = prob.eff.y))
# 
# rhc.model.noeff <- data.frame(A = rhc.used$sim.A,
#                               Y = rhc.used$sim.Y.noeff,
#                               rhc.used[, c("cat1", "imputed.cat2", "age", "surv2md1", "meanbp1", 
#                                            "resp1", "pafi1", "bili1", "dnr1")])
# 
# rhc.model.eff <- data.frame(A = rhc.used$sim.A,
#                             Y = rhc.used$sim.Y.eff,
#                             rhc.used[, c("cat1", "imputed.cat2", "age", "surv2md1", "meanbp1", 
#                                          "resp1", "pafi1", "bili1", "dnr1")])

res.rhcSimSubFtr.dr.AFrst5.YLnrFrst5 <- 
  foreach(i = start:end,
          .combine = "rbind") %dopar% {
            ...
            final.tree.eff <- EstDr.CvMethod1.MltLamda(data.used         = rhc.eff.train,
                                                       tree.list         = seq.created.eff$tree.list, 
                                                       lambda.used       = c(1:3, qchisq(0.95, 1)),
            ...
            # size of final tree
            size.tree <- sum(final.tree.eff[[1]][[1]]$frame$var == "<leaf>")
            size.tree <- c(size.tree, sum(final.tree.eff[[1]][[2]]$frame$var == "<leaf>"))
            size.tree <- c(size.tree, sum(final.tree.eff[[1]][[3]]$frame$var == "<leaf>"))
            size.tree <- c(size.tree, sum(final.tree.eff[[1]][[4]]$frame$var == "<leaf>"))
            ...
            eval.final.eff <- eval.measures.eff(final.tree   = final.tree.eff[[1]][[4]],
            ...
            final.tree.noeff <- EstDr.CvMethod1.MltLamda(data.used         = rhc.noeff.train,
            ...
            # size of final tree
            size.tree <- sum(final.tree.noeff[[1]][[1]]$frame$var == "<leaf>")
            size.tree <- c(size.tree, sum(final.tree.noeff[[1]][[2]]$frame$var == "<leaf>"))
            size.tree <- c(size.tree, sum(final.tree.noeff[[1]][[3]]$frame$var == "<leaf>"))
            size.tree <- c(size.tree, sum(final.tree.noeff[[1]][[4]]$frame$var == "<leaf>"))
            ...
            eval.final.noeff <- eval.measures.eff(final.tree   = final.tree.noeff[[1]][[4]],
...
save(res.rhcSimSubFtr.dr.AFrst5.YLnrFrst5, file = paste0("../Data/AppendixD4/RhcSimSubFtrDrAFrst5YLnrFrst5.RData"))
