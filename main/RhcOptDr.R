...
#####################################################################################################################
t0 <- Sys.time()
seq.created.drInnd.cv1 <- create.sequence(data.used         = rhc.model.train,
                                          est.used          = "DR",
                                          type.var          = "bin",
                                          propsc.mod.loc    = "node",
                                          propsc.mthd       = "GLM",
...
final.tree.drInnd.cv1 <- EstDr.CvMethod1(data.used         = rhc.model.train,
                                         tree.list         = seq.created.drInnd.cv1$tree.list,
...
# time
t.drInnd.step.12 <- as.numeric(difftime(t1, t0, units = "secs"))
t.drInnd.step.3  <- as.numeric(difftime(t2, t1, units = "secs"))
print("DR")

save(seq.created.drInnd.cv1, final.tree.drInnd.cv1,
     t.drInnd.step.12, t.drInnd.step.3,
     file = "../Data/main/RhcOptDr.RData")
