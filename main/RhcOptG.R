...
#####################################################################################################################
t0 <- Sys.time()
seq.created.gInnd.cv1 <- create.sequence(data.used         = rhc.model.train,
                                         est.used          = "G",
                                         type.var          = "bin",
                                         adj.mod.loc       = "node",
                                         adj.mthd          = "GLM",
...
final.tree.gInnd.cv1 <- EstG.CvMethod1(data.used         = rhc.model.train,
                                       tree.list         = seq.created.gInnd.cv1$tree.list,
...
# time
t.gInnd.step.12 <- as.numeric(difftime(t1, t0, units = "secs"))
t.gInnd.step.3  <- as.numeric(difftime(t2, t1, units = "secs")) 
print("G")

save(seq.created.gInnd.cv1, final.tree.gInnd.cv1,
     t.gInnd.step.12, t.gInnd.step.3,
     file = "../Data/main/RhcOptG.RData")
